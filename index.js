...
class p20hdInstance extends InstanceBase {
	...
	init_tcp() {
		...
		if (this.config.host) {
			...
			this.socket.on('data', (receivebuffer) => {
				...
				} else if (pipeline.includes(this.CONTROL_ACK)) {
					// When ACK is received
					if (this.auth.sendUserId && !this.auth.accepted) {
						//if the userid was sent, but connection isn't authenticated, send the password
						this.initLoginPassword()
						this.sendCommand('VER')
					}
				} else if (pipeline.includes('VER') && this.auth.sendPassword && !this.auth.accepted) {
					//if the pipeline contains the version string, the password command was sent and authentication is not accepted, we are now accepted
				...
				if (pipeline.includes(';')) {
					// We already processed ACK and NACK, so if there are any commands left, they will end with ;
	...
	processResponse(response) {
		...
		switch (category) {
			...
			case 'QAX': //get availability of audio clip at asked position (we are iterating over all 16 position one at a time)
				...
				if (this.data.audio_index > 15) {
					this.data.audio_index = 0
				}
			...
			case 'QSX': //get availability of still image at asked position (we are iterating over all 16 position one at a time)
				...
				if (this.data.still_index > 15) {
					this.data.still_index = 0
				}
	...
	sendPollCommands(pollCmds = []) {
		if (this.socket !== undefined && this.socket.isConnected) {
			...
			pollCmds.forEach((cmd) => {
				if (cmd == 'QAX' && this.data.audio_next) {
					//catch QAX command to add parameter
					cmd = cmd + ':' + (this.data.audio_index + 1)
					this.data.audio_next = false
				} else if (cmd == 'QSX' && this.data.still_next) {
					//catch QSX command to add parameter
					cmd = cmd + ':' + (this.data.still_index + 1)
					this.data.still_next = false
				} else if (cmd == 'QNC' && this.data.selected_playlist >= 0 && this.data.selected_playlist <= 8) {
					//catch QNC command to add parameter
	...
	initActions() {
		let actions = {
			...
			shuttle: {
				...
				callback: async (event) => {
					this.sendCommand(
						'SHT:' + event.options.direction.replace('for', '').replace('rev', '-') + event.options.speed
					)
				},
			},
			playback_speed: {
	...
	initVariables() {
		...
		variables.push({
			variableId: 'selected_clip_playlist_length',
			name: 'Number Of Clips In Playlist Containing Selected Clip',
		})
